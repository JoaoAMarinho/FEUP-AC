...
def pca(df, cols, table):
    ...
    df_comp = pd.concat(
        [pd.DataFrame(data=components, columns=["Comp1", "Comp2"]), df[["status"]]],
        axis=1,
    )
...
def merge_transactions_clients(dfs):
    ...
    df = pd.merge(
        df, accounts, on="account_id", how="left", suffixes=("_client", "_account")
    )
...
def clustering_kmeans(df, components, N, title):
    ...
    display(
        Markdown(
            f"**KMeans Silhouette Score:**\n- {silhouette_score(components, kmeans.labels_)}"
        )
    )

    clusters_pca_scale = pd.concat(
        [components, pd.DataFrame({"cluster": kmeans.labels_})], axis=1
    )
    cluster_pca_profile = pd.merge(
        df, clusters_pca_scale["cluster"], left_index=True, right_index=True
    )
