...
def calculate_average_commited_crimes(df):
    ...
    df["avg_commited_crimes"] = (
        df[["no._of_commited_crimes_'95", "no._of_commited_crimes_'96"]].mean(axis=1)
        / df["no._of_inhabitants"]
    )
...
def clean_accounts(df, clean=True):
    ...
    df.rename(
        columns={"district_id": "account_district_id", "date": "creation_date"},
        inplace=True,
    )
...
def clean_districts(df, clean=True):
    ...
    # Criminality growth
    df["criminality_growth"] = (
        df["no._of_commited_crimes_'96"] - df["no._of_commited_crimes_'95"]
    ) / df["no._of_inhabitants"]

    # Unemployment growth
    df["unemployment_growth"] = (
        df["unemploymant_rate_'96"] - df["unemploymant_rate_'95"]
    )
...
def clean_transactions(df, clean=True, op=False, k_symbol=False):
    ...
    df.loc[
        df["k_symbol"] == "sanction interest if negative balance", "k_symbol"
    ] = "Sanction"
    ...
    avg_amount_df = pd.merge(
        credit_amount_mean, withdrawal_amount_mean, on="account_id", how="outer"
    )
    avg_amount_df.fillna(0, inplace=True)

    avg_amount_total = (
        df.groupby(["account_id"]).agg({"amount": ["mean", "min", "max"]}).reset_index()
    )
    avg_amount_total.columns = [
        "account_id",
        "avg_amount_total",
        "min_amount",
        "max_amount",
    ...
    trans_type_count_df = pd.merge(
        credit_counts, withdrawal_counts, on="account_id", how="outer"
    )
    trans_type_count_df.fillna(0, inplace=True)
    trans_type_count_df["credit_ratio"] = trans_type_count_df["num_credits"] / (
        trans_type_count_df["num_credits"] + trans_type_count_df["num_withdrawals"]
    )
    ...
    balance_count_df = (
        ...
        .agg({"balance": ["count", "mean", "min", "max", "std"]})
        .reset_index()
    )
    balance_count_df.columns = [
        "account_id",
        "num_trans",
        "avg_balance",
        "min_balance",
    ...
    if op:
        operation = (
            df.groupby(["account_id", "operation"])
            .agg({"trans_id": ["count"]})
            .reset_index()
        )
        ...
        operation_df = cashC_operation.merge(
            coll_operation, on="account_id", how="outer"
        )
        operation_df = operation_df.merge(
            interest_operation, on="account_id", how="outer"
        )
    ...
    if k_symbol:
        symbol_amount = (
            df.groupby(["account_id", "k_symbol"])
            .agg({"amount": ["mean", "count"]})
            .reset_index()
        )
        symbol_amount.columns = [
            "account_id",
            "k_symbol",
            "symbol_amount_mean",
            "symbol_amount_count",
        ...
        symbol_amount_df = no_symbol.merge(
            household_symbol, on="account_id", how="outer"
        )
        symbol_amount_df = symbol_amount_df.merge(
            statement_symbol, on="account_id", how="outer"
        )
        symbol_amount_df = symbol_amount_df.merge(
            insurance_symbol, on="account_id", how="outer"
        )
        symbol_amount_df = symbol_amount_df.merge(
            sanction_symbol, on="account_id", how="outer"
        )
        symbol_amount_df = symbol_amount_df.merge(
            pension_symbol, on="account_id", how="outer"
        )
...
def clean_columns(df):
    ...
    return df.drop(
        columns=[
            ...
            "no._of_municipalities_with_inhabitants_>10000",
            "no._of_cities",
        ]
    )
...
def extract_other_features(df, clean=True):
    ...
    df["age_at_loan"] = calculate_birth_loan_difference(
        df["birth_date"], df["loan_date"]
    )
